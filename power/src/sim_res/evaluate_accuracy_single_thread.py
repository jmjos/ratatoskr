...
simRep = 1e2  # how often each simulation is repeated
# NBvec = [16, 32, 64]
NBvec = [32, 64]
    ...
    for i in range(int(simRep)):
        ...
        for j in range(Nds):
            ...
            if dsType == 1:
                ...
                ds = ds.as_signed  # ensure that all data streams are signed
            elif dsType == 2:
...
for NB in NBvec:
    ...
    for muxProb, Nds in itertools.product(muxProbs, range(2, maxDs+1)):
        ...
        async_results.append(determine_error(muxProb, Nds, patCount, simRep,
                                             NB))
        i += 1
