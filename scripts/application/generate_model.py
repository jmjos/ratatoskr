...
for t_id in [12, 13, 16, 17, 20, 21]:
    ...
    data_writer.add_possibility(generates_node, 0, 0.1666, [0, 100], 1000,
                                stp2_data_sz, 1, [22, 23, 18, 19, 14, 15])
for t_id in [14, 15, 18, 19, 22, 23]:
    ...
    generates_node = data_writer.add_generates_node(task_node)
    if t_id in [22, 23]:
        data_writer.add_possibility(generates_node, 0, 0.33, [0, 100], 1000,
                                    stp3_data_sz, 2, [32])
    elif t_id in [18, 19]:
        data_writer.add_possibility(generates_node, 0, 0.33, [0, 100], 1000,
                                    stp3_data_sz, 2, [28])
...
for t_id in [24, 28, 32]:
    ...
    generates_node = data_writer.add_generates_node(task_node)
    data_writer.add_possibility(generates_node, 0, 0.25, [0, 100], 1000,
...
for t_id in [25, 26, 27, 29, 30, 31, 33, 34, 35]:
    ...
    if t_id in [33, 29, 25]:
        data_writer.add_possibility(generates_node, 0, 0.33, [0, 100], 1000,
                                    stp1_data_sz, 0, [20, 16, 12])
    elif t_id in [34, 30, 26]:
        data_writer.add_possibility(generates_node, 0, 0.166, [0, 100], 1000,
                                    stp1_data_sz, 0, [20, 16, 12])
        data_writer.add_possibility(generates_node, 1, 0.166, [0, 100], 1000,
                                    stp1_data_sz, 0, [21, 17, 13])
...
map_writer.add_bindings(list(range(0, 36)), list(range(0, 36)))
